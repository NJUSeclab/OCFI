...
def compareFuncs(groundTruth, compared, lastBB):
    """
    ...
    for func in compared:
        if func not in groundTruth:
            ...
            for f in groundTruth:
                if f <= func:
                    match_f = max(match_f,f)
            logging.error("[Func Start False Positive #{0}]:Function Start 0x{1:x} not in Ground Truth.".format(falsePositive, func))
            if match_f not in lastBB:
                logging.error("Func 0x%x not in gt" %(match_f))
            else:
                logging.info("[Match Func]: 0x%x => 0x%x" %(match_f,lastBB[match_f]))
                if func >= lastBB[match_f]:
                    #logging.info("Nounwind Function!")
                    noUnwindCount += 1
                else:
                    #logging.info("Unwind Function!")
                    unwindCount += 1
            falsePositive += 1
        else:
...
#    print("[Result]:Unwind Fucntion number is %d" % (unwindCount))
#    print("[Result]:Nounwind Function number is %d" % (noUnwindCount))
    ...
    total = unwindCount + noUnwindCount
    if total != 0:
        print("[Result]:Unwind %f" %(unwindCount / total))
        print("[Result]:Nounwind %f" %(noUnwindCount / total))
...
def readFuncsLastBB(file,func_list):
    res = dict()
    with open(file,'r') as info_list:
        for info in info_list:
            addr = int(info.strip(),16) + textAddr 
            func = 0x0
            for f in func_list:
                if f <= addr:
                    func = max(func,f)
...
if __name__ == '__main__':
   ...
   # assert options.groundtruth != None, "Please input the ground truth file!"
    ...
    if options.groundtruth == None:
        ...
        LastBB = readFuncsLastBB(options.groundtruth,truthFuncs)

    comparedFuncs = readEh(options.comparedfile)
    compareFuncs(truthFuncs, comparedFuncs,LastBB)
