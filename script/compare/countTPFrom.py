...
def countTruePositive(groundTruth, compared):
    """
    ...
    for func in compared:
        if func not in groundTruth:
            ...
            # logging.info("[Truth Positive Function #%d]:Function Start 0x%x matched, name is %s" %(truePositive,func,func_name_list[func]))
            truePositive += 1
    ...
    # if recall != 0:
    #     print("[Result]:F1_Score %f" % f1_scores(precision,recall))
    return truePositive
...
def readEh(file):
    res = set()
    with open(file,'r') as info_list:
        for info in info_list:
            res.add(int(info.strip(),16))
            #print(info.strip())
    return res
'''
    ...
    with open(binary, 'rb') as open_file:
        ...
        for sym in symsec.iter_symbols():
            if 'STT_FUNC' == sym['st_info']['type'] and sym['st_value'] != 0x0 and \
                ...
                func_name_list[sym['st_value']] = sym.name
    return result
...
if __name__ == '__main__':
    ...
    parser.add_option("-e", "--ehfile", dest = "ehfile", action = "store", \
            type = "string", help = "eh file path", default = None)
    parser.add_option("-n", "--nofuncfile", dest = "nofuncfile", action = "store", \
            type = "string", help = "no func file path", default = None)
    parser.add_option("-f", "--funcfile", dest = "funcfile", action = "store", \
            type = "string", help = "func file path", default = None)
    parser.add_option("-b", "--binaryFile", dest = "binaryFile", action = "store", \
            type = "string", help = "binary file path", default = None)

    (options, args) = parser.parse_args()

    assert options.ehfile != None, "Please input the eh file!"
    assert options.nofuncfile != None, "Please input the nofunc file!"
    assert options.funcfile != None, "Please input the func file!"
    ...
    if PIE:
        disassembler_base_addr = getPIEBaseOffset(options.funcfile)
    getLinkerFunctionAddr(options.binaryFile)

    truthFuncs = readFuncsFromSyms(options.binaryFile)
    ehFuncs = readEh(options.ehfile)

    mModule1 = blocks_pb2.module()
    mModule2 = blocks_pb2.module()
    try:
        f1 = open(options.funcfile, 'rb')
        mModule1.ParseFromString(f1.read())
        f1.close()
        f2 = open(options.nofuncfile, 'rb')
    ...
    if "ghidra" in options.funcfile.lower() and PIE:
        doubleCheckGhidraBase(mModule2)
        
    funcFuncs = readFuncs(mModule1, False)
    if PIE:
        disassembler_base_addr = getPIEBaseOffset(options.nofuncfile)


    nofuncFuncs = readFuncs(mModule2,False)

    ehTruePositive = countTruePositive(truthFuncs,ehFuncs)
    totalTruePositive = countTruePositive(truthFuncs,funcFuncs)
    tmp  = countTruePositive(truthFuncs,nofuncFuncs)
    callTruePositive = tmp - ehTruePositive
    funcTruePositive = totalTruePositive - ehTruePositive - callTruePositive
    print("T: %d %d %d" % (ehTruePositive,totalTruePositive,tmp))
    print("[Result]:The eh TruePositive is %d : %f" % (ehTruePositive, ehTruePositive / totalTruePositive))
    print("[Result]:The func TruePositive is %d : %f" % (funcTruePositive, funcTruePositive / totalTruePositive))
    print("[Result]:The call TruePositive is %d : %f" % (callTruePositive, callTruePositive / totalTruePositive))
