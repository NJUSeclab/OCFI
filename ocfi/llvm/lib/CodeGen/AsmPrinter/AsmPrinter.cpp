...
void AsmPrinter::emitFunctionBody() {
  ...
  // find the Obfuscate begin block
  MachineBasicBlock* obfuscated_bb = nullptr;
  for (MachineBasicBlock& mbb : *MF) {
    if (mbb.isObfuscatedBegin()) {
      obfuscated_bb = &mbb;
      break;
    }
  }
  assert(obfuscated_bb != nullptr && "Can't find the obfuscated starting block!");
  MBBSectionRanges[MF->front().getSectionIDNum()] =
    MBBSectionRange{obfuscated_bb->getSymbol(), CurrentFnEnd};
...
void AsmPrinter::emitBasicBlockStart(const MachineBasicBlock &MBB) {
  ...
  // || MBB.isObfuscatedBegin() binpang. add
  if ((MBB.isBeginSection() && !MBB.isEntryBlock())
      || MBB.isObfuscatedBegin()
  ){
    for (const HandlerInfo &HI : Handlers)
